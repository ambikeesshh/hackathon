...
export default function RoomPage() {
  ...
  if (!room) {
    return (
      <div className="flex h-64 flex-col items-center justify-center" style={{ color: "var(--text-subtle)" }}>
        <p className="text-lg font-medium">Room not found</p>
        <button onClick={() => navigate("/dashboard")} className="mt-3 text-sm font-semibold text-blue-600">
          ← Back to Dashboard
        </button>
      </div>
    );
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      await addLog(room.id, authUser.uid, isFree || isReserved ? "occupied" : "free", room.note || "");
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-10">
      ...
      <div
        ...
        style={{ borderColor: "var(--border)", background: "var(--bg-elevated)", boxShadow: "0 8px 24px rgba(var(--shadow), 0.09)" }}
      >
        {/* Status indicator */}
        <div className={`mb-6 rounded-xl p-4 text-center ${isFree ? "bg-emerald-50" : isReserved ? "bg-amber-50" : "bg-red-50"}`}>
          <div
            className={`mx-auto mb-2 flex h-14 w-14 items-center justify-center rounded-full ${
              isFree ? "bg-emerald-100" : isReserved ? "bg-amber-100" : "bg-red-100"
            }`}
          >
            {isFree ? (
              <svg viewBox="0 0 24 24" className="h-7 w-7 text-emerald-700" fill="none">
              ...
              <svg viewBox="0 0 24 24" className="h-7 w-7 text-red-600" fill="none">
                <circle cx="12" cy="12" r="7" stroke="currentColor" strokeWidth="2" />
                <path d="M12 8.5v4.5" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" />
                <circle cx="12" cy="16.4" r="1" fill="currentColor" />
              </svg>
            )}
          </div>
          <p className={`text-2xl font-black ${isFree ? "text-emerald-700" : isReserved ? "text-amber-700" : "text-red-700"}`}>
            {isFree ? "Available" : isReserved ? "Reserved" : "Occupied"}
          </p>
        </div>

        <h1 className="mb-1 text-center text-2xl font-black" style={{ color: "var(--text)" }}>{room.name}</h1>
        <p className="mb-1 text-center text-sm" style={{ color: "var(--text-muted)" }}>
          {[room.building || "Building N/A", room.floor ? `Floor ${room.floor}` : "Floor N/A", room.type || "Type N/A", `Cap ${room.capacity || 0}`].join(" • ")}
        </p>
        <p className="mb-6 text-center text-sm" style={{ color: "var(--text-subtle)" }}>
          Last updated: {timeAgo(room.updatedAt)}
        </p>

        {room.note && (
          <div className="mb-4 rounded-lg border px-4 py-2.5 text-xs" style={{ borderColor: "var(--border)", background: "var(--bg-soft)", color: "var(--text-muted)" }}>
            {room.note}
          </div>
        )}
        ...
        <div className="mb-4">
          <ReservationUI room={room} authUser={authUser} canManage={canToggle} />
        </div>
        ...
        <div className="space-y-3">
          {canToggle && (
            ...
            >
              {toggling ? "Updating…" : isFree || isReserved ? "Start Session" : "End Session"}
            </button>
          )}
