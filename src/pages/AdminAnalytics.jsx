...
export default function AdminAnalytics() {
  ...
  const usageByResourceData = useMemo(() => {
    return rooms
      .map((room) => {
        const roomLogs = logs.filter((log) => log.resourceId === room.id);
        const occupiedCount = roomLogs.filter(
          (log) => log.action === STATUS.OCCUPIED || log.newStatus === STATUS.OCCUPIED
        ).length;
        return {
          name: room.name.length > 15 ? room.name.slice(0, 15) + '...' : room.name,
          fullName: room.name,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="rounded-2xl border-2 border-slate-900 bg-white p-4 shadow-[4px_4px_0px_0px_#0f172a]">
          ...
          <p className="mt-1 font-mono text-xs uppercase text-slate-500">Total Resources</p>
        </div>
        <div className="rounded-2xl border-2 border-slate-900 bg-green-100 p-4 shadow-[4px_4px_0px_0px_#0f172a]">
          ...
          <p className="mt-1 font-mono text-xs uppercase text-green-700">Currently Free</p>
        </div>
        ...
        <div className="rounded-2xl border-2 border-slate-900 bg-blue-100 p-4 shadow-[4px_4px_0px_0px_#0f172a]">
          <p className="text-3xl font-black text-blue-600">{stats.occupancyRate}%</p>
          <p className="mt-1 font-mono text-xs uppercase text-blue-700">Occupancy Rate</p>
        </div>
      </div>
        ...
        <div className="max-h-64 overflow-y-auto">
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-6 py-3 text-left font-mono text-[10px] uppercase text-slate-500">Time</th>
                <th className="px-6 py-3 text-left font-mono text-[10px] uppercase text-slate-500">Room</th>
                <th className="px-6 py-3 text-left font-mono text-[10px] uppercase text-slate-500">Action</th>
                <th className="px-6 py-3 text-left font-mono text-[10px] uppercase text-slate-500">By</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {recentActivity.map((log) => (
                <tr key={log.id || Math.random()} className="hover:bg-slate-50">
                  ...
                  <td className="whitespace-nowrap px-6 py-3 text-sm font-bold text-slate-900">
                  ...
                  <td className="whitespace-nowrap px-6 py-3 font-mono text-xs text-slate-500">
                    {log.changedByName || log.changedBy?.slice(0, 8) || 'System'}
                  </td>
                </tr>
              ))}
            </tbody>
