...
export default function FloorPlanView() {
  ...
  const floors = useMemo(() => {
    return [...new Set(rooms.map((r) => r.floor).filter(Boolean))].sort((a, b) => a - b);
  }, [rooms]);
  ...
  const getStatusColor = (room) => {
    ...
    if (status === STATUS.FREE) return "bg-green-500";
    if (status === STATUS.RESERVED) return "bg-yellow-500";
    return "bg-red-500";
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black" style={{ color: isDark ? "#f0e7dc" : "#201a15" }}>
          Floor Plan
        </h1>
        <button
      ...
      >
        <div className="mb-4 flex gap-6 text-sm font-bold">
          ...
          <div className="flex items-center gap-2">
            ...
            <span style={{ color: isDark ? "#a0a0a0" : "#625041" }}>Reserved</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="h-4 w-4 rounded-full bg-red-400" />
            <span style={{ color: isDark ? "#a0a0a0" : "#625041" }}>Occupied</span>
          </div>
        </div>
          ...
          {filteredRooms.map((room) => {
            ...
            return (
              <button
                ...
                className={`group relative flex flex-col items-center justify-center rounded-xl border-2 p-3 py-4 transition-all hover:-translate-y-1 ${
        ...
        {filteredRooms.length === 0 && (
          <div className="py-12 text-center">
            <p className="text-lg font-medium" style={{ color: isDark ? "#a0a0a0" : "#625041" }}>
