      ...
      <div className="flex w-full items-center justify-between px-4 py-3">
        <div className="flex items-center gap-3">
          <Link to="/" className="flex items-center gap-3">
            <div
              className={`flex h-9 w-9 items-center justify-center rounded-xl ${isDark ? 'bg-yellow-600' : 'bg-yellow-400'} text-sm font-black ${isDark ? 'text-slate-100' : 'text-slate-900'} shadow-sm ring-1 ring-slate-900/20`}
            >
              CS
            </div>
            <div>
        ...
        <div className="flex items-center gap-4">
          ...
          {authUser && (
            <span
              className={`hidden rounded-full border-2 ${isDark ? 'border-slate-600 bg-slate-800 text-slate-300' : 'border-slate-900 bg-white text-slate-700'} px-3 py-1 text-xs font-bold uppercase sm:block`}
            >
              {authUser.role}
            </span>
          )}
          {authUser && (
            <span
              className={`hidden max-w-[120px] truncate text-sm font-bold ${isDark ? 'text-slate-400' : 'text-slate-600'} sm:block`}
            >
              {authUser.name}
            </span>
          )}
