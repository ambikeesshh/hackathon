...
function BookingCard({ booking, onRespond }) {
  ...
  const statusColors = {
    ...
    approved: isDark ? "bg-green-900/30 text-green-400" : "bg-green-100 text-green-700",
    declined: isDark ? "bg-red-900/30 text-red-400" : "bg-red-100 text-red-700",
  };

  const canRespond = authUser?.role === ROLES.FACULTY || authUser?.role === ROLES.ADMIN;

  return (
    <div className="rounded-xl border p-3" style={{ borderColor: "var(--border)", background: "var(--bg-elevated)" }}>
      <div className="flex items-start justify-between">
        <div>
          <h4 className="font-semibold" style={{ color: "var(--text)" }}>{booking.resourceName}</h4>
        ...
        <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${statusColors[booking.status]}`}>
          {booking.status}
        </span>
      </div>

      {booking.purpose && (
        <p className="mt-2 text-sm" style={{ color: "var(--text-muted)" }}>{booking.purpose}</p>
      )}
...
export default function BookingList({ resourceId }) {
  ...
  if (loading) {
    return <div className="flex h-20 items-center justify-center">
  ...
  if (!bookings.length) {
    return (
      <div className="rounded-xl border p-4 text-center" style={{ borderColor: "var(--border)", background: "var(--bg-soft)" }}>
        <p className="text-sm" style={{ color: "var(--text-muted)" }}>No booking requests</p>
      </div>
    );
  }
